...
async function checkRateLimits(
  ...
  if (ratelimit) {
    ...
    // Calculate the time when execution will continue
    const continueTime = new Date(new Date().getTime() + waitTime)
    const continueTimeFormatted =
      continueTime.toISOString().replace('T', ' ').substring(0, 19) + ' UTC'

    core.info(
      `Waiting ${waitTimeSeconds.toFixed(3)} seconds (${formattedWaitTime}) to prevent the search API rate limit`
    )
    core.info(`Will continue at ${continueTimeFormatted}`)
...
async function paginateSearchQuery(
  client: Octokit,
  searchQuery: string,
  isEnterpriseServer: boolean,
  searchRepos: boolean
...
async function getActionInfo(
  ...
  if ('name' in yaml && 'download_url' in yaml) {
    ...
    result.owner = owner
    result.repo = repo
    result.path = actionFilePath.includes('/')
      ? path.dirname(actionFilePath)
      : ''
