...
**1000 Result Limit**: GitHub's code search API (both REST and GraphQL) has a hard limit of 1000 results per search query. This is a platform limitation that cannot be bypassed:
- The REST API code search returns a maximum of 1000 results
- GitHub's GraphQL API does not support code search (`type: CODE` is not available)
- For organizations with more than 1000 action files or workflows, only the first 1000 results will be discovered

**Workaround**: For large organizations, consider:
- Splitting repositories across multiple organizations
- Using repository naming conventions to help locate actions
- Manually tracking actions in repositories beyond the 1000 result limit
