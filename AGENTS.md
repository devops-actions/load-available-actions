...
- The integration test workflows (`testing.yml`, `test-big-organization.yml`) and the publish test job all use `secrets.PAT`. If they fail instantly with `Bad credentials`, the PAT has expired — rotate it under Settings → Secrets → Actions → `PAT`.
